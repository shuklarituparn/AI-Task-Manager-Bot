...
async def delete_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text=f"Убираем {r.get(index_key)} из списка")
    await context.bot.send_message(chat_id=update.effective_chat.id, text=f"Поздравляю тебе при заполнении задачи")
    r.delete(index_key)
...
async def delete_all_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if task_keys:
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f"Deleted all {deleted_count} tasks.")
    else:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="No tasks found to delete.")
